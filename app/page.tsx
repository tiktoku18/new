...
const generateVideoData = (index: number): VideoData => {
...
const generateAdData = (index: number): AdData => {
  const adTypes = [
    {
...
export default function TikTokClone() {
  const [videos, setVideos] = useState<(VideoData | { type: "ad"; data: AdData })[]>([])
  ...
  const [inboxCount, setInboxCount] = useState<number>(3)
  const [isScrolling, setIsScrolling] = useState<boolean>(false)
  const scrollTimeoutRef = useRef<NodeJS.Timeout | null>(null)

  const loadMoreVideos = useCallback(() => {
    const newVideos: (VideoData | { type: "ad"; data: AdData })[] = []
      ...
      setAdTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          if (containerRef.current) {
            const nextVideoTop = (currentVideoIndex + 1) * containerRef.current.clientHeight
  ...
  // Inbox counter effect
  useEffect(() => {
    const inboxInterval = setInterval(() => {
      setInboxCount((prev) => prev + 2)
    }, 2000)
  ...
  const handleScroll = useCallback(() => {
    ...
    if (newIndex !== currentVideoIndex && newIndex < videos.length) {
      ...
      videoRefs.current.forEach((video, idx) => {
        if (video) {
          if (idx === newIndex && "videoUrl" in videos[idx]) {
            video.play().catch(() => {
              setIsPlaying((prev) => {
                const newState = [...prev]
                newState[idx] = false
  ...
  }, [currentVideoIndex, videos, loadMoreVideos, stopAdTimer])

  // Effect to start ad timer when not scrolling
  useEffect(() => {
    if (!isScrolling && videos[currentVideoIndex] && videos[currentVideoIndex].type === "ad") {
      const timer = setTimeout(() => {
        startAdTimer()
      }, 200)
      return () => clearTimeout(timer)
    }
  }, [isScrolling, currentVideoIndex, videos, startAdTimer])
  ...
  const handleCommentLike = (videoIndex: number, commentIndex: number) => {
    setVideos((prev) =>
      prev.map((item, i) => {
        if (i === videoIndex && "comments" in item) {
          ...
          >
            {item.type === "ad" ? (
              <Link href={item.data.link} className="block h-full">
                ...
                >
                  <img
                    src={`/placeholder.svg?height=600&width=400&text=${item.data.title.split(" ")[0].toUpperCase()}`}
                    alt="Advertisement"
                    className="w-full h-full object-cover"
                  />

                  {/* Ad Timer */}
                  {index === currentVideoIndex && !isScrolling && adTimer && (
                    <div className="absolute top-20 right-4 bg-black bg-opacity-70 text-white px-3 py-2 rounded-full text-sm font-bold">
                      {adTimeLeft}s
                    </div>
                  )}
        ...
        <div className="flex items-center justify-around py-2">
          ...
          <Link href="https://example.com" className="flex flex-col items-center p-2 relative">
            ...
            <div className="absolute -top-1 -right-1 bg-red-500 rounded-full w-4 h-4 flex items-center justify-center">
              <span className="text-xs text-white">{inboxCount}</span>
            </div>
          </Link>
          <Link href="https://example.com" className="flex flex-col items-center p-2">
          ...
          <div className="bg-white rounded-t-3xl w-full max-h-[70vh] overflow-hidden">
            ...
            <div className="overflow-y-auto max-h-96 p-4">
              {videos[currentVideoIndex] &&
                "comments" in videos[currentVideoIndex] &&
                (videos[currentVideoIndex] as VideoData).comments.map((comment, commentIndex) => (
